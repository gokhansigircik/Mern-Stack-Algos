...
function musicRuntime(busDuration, songDurations) {
  // your code here
  const originalIndex = {};
  const sortedSongs = [...songDurations].sort((a,b) => a-b);
  

  for(let i=0; i<songDurations.length; i++){
      originalIndex[i] = songDurations[i];
  }

  for(let i=sortedSongs.length-1; i>=0; i--){
      const target = busDuration - 30 - sortedSongs[i];
      if(Object.values(originalIndex).indexOf(target) > -1){
          return [Object.values(originalIndex).indexOf(target), Object.values(originalIndex).indexOf(sortedSongs[i])]
      }
  }
  ...
  for (let i = 0; i < songDurations.length; i++) {
    for (let j = i + 1; j < songDurations.length; j++) {
      if (songDurations[i] + songDurations[j] === target) {
        if (songDurations[i] > longestDuration || songDurations[j] > longestDuration) {
          longestDuration = Math.max(songDurations[i], songDurations[j]);
