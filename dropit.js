// more explicitly, a function call expression evaluates to its return



/*
Input: arr, callback
Output: arr (with elements removed)
Remove every element in the array, starting from idx 0,
until the callback function returns true when passed the
iterated element.
Return an empty array if the callback never returns true
*/
//            v
const arr1 = [1, 4, 3, 6, 9, 3];
const callback1 = (elem) => {
    return elem > 5;
};
const expected1 = [6, 9, 3];




const arr2 = [1, 4, 3, 6, 9, 3];
const callback2 = (elem) => {
    return elem > 2;
};
const expected2 = [4, 3, 6, 9, 3];




const arr3 = [1, 4, 3, 6, 9, 3];
const callback3 = (elem) => false;
const expected3 = [];


function dropIt(arr, callback) { 

    // Loop through array
    for (let i = 0; i < arr.length ; i++ ){

    // Use Callback for logic
        if(callback(arr[i])){
            // Splice if true
            arr.splice(0,i);
            return arr;
        }
    }
    // return statement
    return [];
}

function dropIt(arr, callback) { 
    //  ...
    while(arr.length > 0){
        if(!callback(arr[0])){
            arr.shift();
        }
        else{
            return arr;
        }
    }
    return arr
}

/*
loop through the given array
pass the index element into the callback function:
    if callback return true:
        return the remaining array;
    if callback return false:
        remove the element from the given array
after iteration if return never trigger within the loop, return empty array;
*/

// app.get("/api", (req, res) => {
    //     res.json
    // })
    

console.log(dropIt(arr1, callback1));
console.log(dropIt(arr2, callback2));
console.log(dropIt(arr3, callback3));